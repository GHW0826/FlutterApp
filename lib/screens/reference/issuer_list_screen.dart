...
class _IssuerListScreenState extends State<IssuerListScreen> {
  ...
  Future<void> _loadItems() async {
    ...
    try {
      final list = await issuerApi.getList();
      if (!mounted) return;
      setState(() {
        _items = list;
        _loading = false;
  ...
  Future<void> _onEdit() async {
    ...
    final result = await navigator.push<IssuerEditResult?>(
      MaterialPageRoute(
        builder: (_) =>
            IssuerEditScreen(initialData: initial, parentOptions: _items),
      ),
    );
    ...
    try {
      final updated = switch (result.action) {
        ...
        IssuerSaveAction.create => await issuerApi.patch(result.data),
      };
      await _loadItems();
      if (!mounted) return;
  ...
  Future<void> _onNew() async {
    final result = await Navigator.of(context).push<IssuerEditResult?>(
      MaterialPageRoute(
        builder: (_) => IssuerEditScreen(parentOptions: _items),
      ),
    );
    if (result == null || !mounted) return;
    try {
