...
class UnderlyingThemeListScreen extends StatefulWidget {
  ...
  @override
  State<UnderlyingThemeListScreen> createState() =>
      _UnderlyingThemeListScreenState();
}
...
class _UnderlyingThemeListScreenState extends State<UnderlyingThemeListScreen> {
  ...
  List<ListItemModel> _getFilteredItems() {
    ...
    return _items.where((item) {
      ...
      final matchCode =
          searchCode && (item.subtitle?.toLowerCase().contains(k) ?? false);
      return matchName || matchCode;
    }).toList();
  }
  ...
  UnderlyingBondFormData _initialBondFormFromItem(ListItemModel item) {
    ...
    final marketName =
        _bondMarketMasters
            .where((e) {
              final code = (e.subtitle?.trim().isNotEmpty ?? false)
                  ? e.subtitle!.trim()
                  : e.id;
  ...
  Future<UnderlyingBondFormData?> _loadInitialForBondEdit(
    ListItemModel item,
  ) async {
    try {
      final marketMasters = await bondApi.getList();
      final marketCode = _marketCodeOf(item);
      final detail = marketCode.isEmpty
          ? null
          : await bondApi.getById(marketCode);
      ...
      return (saved ?? _initialBondFormFromItem(item)).copyWith(
        ...
        ccy: Ccy.fromApi(detail?.currencyCode) ?? saved?.ccy,
      );
    } catch (_) {
      if (mounted) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(const SnackBar(content: Text('?듭떊?ㅻ쪟')));
      }
      return null;
    }
  }
  ...
  Future<Ccy?> _resolveMarketCcy(String marketDataCode) async {
    ...
    return Ccy.fromApi(detail?.currencyCode);
  }
  ...
  Future<void> _onEdit() async {
    ...
    final result = await navigator.push<UnderlyingBondFormData?>(
      MaterialPageRoute(
        builder: (_) => UnderlyingBondEditScreen(
          initialData: initial,
          marketDataOptions: _bondMarketMasters,
              ...
              Row(
                children: [
                  ...
                  Checkbox(
                    ...
                    onChanged: (v) =>
                        setState(() => _marketCodeChecked = v ?? true),
                  ),
                  GestureDetector(
                    onTap: () => setState(
                      () => _marketCodeChecked = !_marketCodeChecked,
                    ),
  ...
  Widget _buildListView(List<ListItemModel> items) {
    if (items.isEmpty) {
      return Center(
        child: Text(
          _appliedKeyword.isEmpty ? '紐⑸줉??鍮꾩뼱 ?덉뒿?덈떎.' : '寃??寃곌낵媛 ?놁뒿?덈떎.',
        ),
      );
    }
