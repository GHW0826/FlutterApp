...
import 'bond_curve_build_method_edit_screen.dart';

class BondCurveBuildMethodListScreen extends StatefulWidget {
  const BondCurveBuildMethodListScreen({super.key});

  @override
  State<BondCurveBuildMethodListScreen> createState() =>
      _BondCurveBuildMethodListScreenState();
}

class _BondCurveBuildMethodListScreenState
    extends State<BondCurveBuildMethodListScreen> {
  ...
  Future<void> _loadItems() async {
    ...
    try {
      final list = await bondCurveApi.getBuildMethodList();
      if (!mounted) return;
      setState(() {
        _items = list;
        _loading = false;
  ...
  Future<void> _onNew() async {
    final result = await Navigator.of(context)
        .push<BondCurveBuildMethodFormData?>(
          MaterialPageRoute(
            builder: (_) => const BondCurveBuildMethodEditScreen(),
          ),
        );
    if (result == null || !mounted) return;
    try {
      final created = await bondCurveApi.createBuildMethod(result);
      await _loadItems();
      if (!mounted) return;
      setState(() => _selectedId = created.id);
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('BondCurveBuildMethod created.')),
      );
    } catch (e) {
      if (!mounted) return;
  ...
  Future<void> _onEdit() async {
    ...
    try {
      final initial = await bondCurveApi.getBuildMethodById(_selectedId!);
      if (!mounted) return;
      if (initial == null) {
        ScaffoldMessenger.of(context).showSnackBar(
      ...
      final result = await Navigator.of(context)
          .push<BondCurveBuildMethodFormData?>(
            MaterialPageRoute(
              builder: (_) =>
                  BondCurveBuildMethodEditScreen(initialData: initial),
            ),
          );
      if (result == null || !mounted) return;
      final updated = await bondCurveApi.updateBuildMethod(result);
      await _loadItems();
      if (!mounted) return;
      setState(() => _selectedId = updated.id);
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('BondCurveBuildMethod updated.')),
      );
    } catch (e) {
      if (!mounted) return;
      ScaffoldMessenger.of(
  ...
  Future<void> _onDelete() async {
    if (_selectedId == null) return;
    try {
      await bondCurveApi.deleteBuildMethod(_selectedId!);
      await _loadItems();
      if (!mounted) return;
      setState(() => _selectedId = null);
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('BondCurveBuildMethod deleted.')),
      );
    } catch (e) {
      if (!mounted) return;
      ScaffoldMessenger.of(
