...
class _MarketThemeListScreenState extends State<MarketThemeListScreen> {
  ...
  Future<void> _onNew() async {
        ...
        try {
          ...
          if (mounted) {
            ...
            ScaffoldMessenger.of(
              context,
            ).showSnackBar(const SnackBar(content: Text('Bond 저장됨')));
          }
        } catch (e) {
          if (mounted) {
            ScaffoldMessenger.of(
    ...
    ScaffoldMessenger.of(
      context,
    ).showSnackBar(SnackBar(content: Text('New: ${widget.themeLabel} 추가됨')));
  }
    ...
    if (widget.themeId == 'bond') {
      try {
        ...
        if (initial == null) {
          ScaffoldMessenger.of(
            context,
          ).showSnackBar(const SnackBar(content: Text('항목을 찾을 수 없습니다.')));
          return;
        }
        final result = await Navigator.of(context).push<BondFormData?>(
          MaterialPageRoute(
            builder: (_) => BondEditScreen(initialData: initial),
          ),
        );
        if (result != null && mounted) {
          ...
          if (mounted) {
            ...
            ScaffoldMessenger.of(
              context,
            ).showSnackBar(const SnackBar(content: Text('Bond 수정됨')));
          }
        }
      } catch (e) {
        if (mounted) {
          ScaffoldMessenger.of(
            context,
          ).showSnackBar(SnackBar(content: Text('조회/수정 실패: $e')));
        }
      }
      return;
    }
    ...
    if (widget.themeId == 'bond') {
      try {
        ...
        if (mounted) {
          ...
          ScaffoldMessenger.of(
            context,
          ).showSnackBar(const SnackBar(content: Text('Delete: 선택 항목 삭제됨')));
        }
      } catch (e) {
        if (mounted) {
          ScaffoldMessenger.of(
